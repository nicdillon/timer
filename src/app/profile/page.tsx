    ...
    <div className="flex flex-col items-center justify-center h-screen w-screen">
      {user && (
        <Paper className="p-10 text-center flex flex-col justify-center w-auto min-w-1/2 items-cente rounded-lg shadow-lg text-2xl h-auto text-black gap-4">
          ...
          <Link prefetch={false} className="block text-[var(--accent)] hover:text-[var(--accent)]  rounded px-4 py-2 text-center underline" href="/api/auth/logout">Logout</Link>
        </Paper>
      )}
      {!user && (
        <Paper className="p-10 m-10 text-center flex flex-col justify-center w-auto max-w-1/2 items-cente rounded-lg shadow-lg text-2xl h-auto text-black gap-4">
          ...
          <Link prefetch={false} className="w-auto text-[var(--accent)] hover:text-[var(--accent)] border border-[var(--accent)] rounded px-4 py-2 text-center bg-white bg-opacity-30 backdrop-blur-md shadow-lg" href="/api/auth/login">Login</Link>
        </Paper>)}
    </div>
  );
}